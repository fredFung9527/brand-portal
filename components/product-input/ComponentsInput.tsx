...
function SingleItem({
  ...
  const [factory, setFactory] = useState(value?.factory || '')
  ...
  useEffect(() => {
    debounceCall(() => {
      onChange({axCode, factory, type, remarks, materials})
      onValid(Boolean(axCode && factory && type && materials.length))
    })
  }, [axCode, factory, type, remarks, materials])
    ...
    <Grid container columnSpacing={2} alignItems='flex-end'>
      ...
      <Grid item xs={12} sm={6}>
        <MyTextField
          ...
          value={factory}
          onChange={setFactory}
          error={showError && !Boolean(factory)}
          helperText={showError && !Boolean(factory) && t('error:required')}
          required
          disabled={disabled}
        />
      </Grid>
